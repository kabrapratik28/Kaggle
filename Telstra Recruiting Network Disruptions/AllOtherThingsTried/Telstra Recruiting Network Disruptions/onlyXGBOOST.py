...
location
'''
location_f = ['is_loc_below_500_not_be_sev_2','count_of_loc']
#OHE
...
with open("selector_ranking_", 'rb') as f:
    selected_feature_ranking = pickle.load(f)

name_of_clms_in_feature_dataset = list(dataset_train_sel_fea.columns.values)

#to be considered 
#total 1588 features
# 1st rank got 213 features
# ranking upto 1375 
feat_and_its_rank = zip(selected_feature_ranking,name_of_clms_in_feature_dataset)
feat_and_rank_map = {}
for i in feat_and_its_rank:
    feat_and_rank_map[i[1]] = i[0]

'''
#feat to be considered
feat_to_be_considered_rank_wise = []
upto_which_rank_fea_consider = 600
for i in feat_and_its_rank:
    if i[0] <= upto_which_rank_fea_consider: 
        feat_to_be_considered_rank_wise.append(i[1])

dataset_train_sel_fea = dataset_train[feat_to_be_considered_rank_wise]
dataset_train_sel_fea_val = dataset_train_sel_fea.values
dataset_test_sel_fea = dataset_test[feat_to_be_considered_rank_wise]
dataset_test_sel_fea_val = dataset_test_sel_fea.values
'''
#======================making features out of importance feat in xgboost ==============
import pickle
with open("feat_xgboost_importance_", 'rb') as f:
    fea_imp_with_score = pickle.load(f)

total_all_cons_by_xg = []
f_importance_wise = []
loc_impotance_wise = []
e_impotance_wise = []
severity_impotance_wise = []
r_impotance_wise = []
for i in fea_imp_with_score:
    if i[0][0] == "f":
        f_importance_wise.append(i[0])
    elif i[0][:3] == "loc":
        loc_impotance_wise.append(i[0])
    elif i[0][0] == "e":
        e_impotance_wise.append(i[0])
    elif i[0][:8] == "severity":
        severity_impotance_wise.append(i[0])
    elif i[0][0] == "r":
        r_impotance_wise.append(i[0])
    total_all_cons_by_xg.append(i[0])
 
#===shows that this are the only feat used by xgboost ...        
#dataset_train_sel_fea = dataset_train_sel_fea[total_all_cons_by_xg]
#dataset_train_sel_fea_val = dataset_train_sel_fea.values
#dataset_test_sel_fea = dataset_test_sel_fea[total_all_cons_by_xg]
#dataset_test_sel_fea_val = dataset_test_sel_fea.values
'''
NumberOfBatchesToConsider = 25
bacthOfTogether = 5
counterToList = 0
for i in range(0,NumberOfBatchesToConsider):
    dataset_train_sel_fea["f_together_xgboost_importance_wise_"+str(i)] = 0  
    dataset_test_sel_fea["f_together_xgboost_importance_wise_"+str(i)] = 0
    for j in range(0,bacthOfTogether):
        #print "=>" + f_importance_wise[counterToList]        
        dataset_train_sel_fea["f_together_xgboost_importance_wise_"+str(i)] = dataset_train_sel_fea["f_together_xgboost_importance_wise_"+str(i)] + dataset_train_sel_fea[f_importance_wise[counterToList]]
        dataset_test_sel_fea["f_together_xgboost_importance_wise_"+str(i)] = dataset_test_sel_fea["f_together_xgboost_importance_wise_"+str(i)] + dataset_test_sel_fea[f_importance_wise[counterToList]]
        counterToList = counterToList + 1
    #print "==========="
        
#====less varienced features ....         
fs = []
for l in range(1,387):
    fs.append('f'+str(l))

remaining_feat = list(set(fs)-set(f_importance_wise))
remaining_feat_and_rank = []
for r in remaining_feat:
    remaining_feat_and_rank.append((r,feat_and_rank_map[r]))
import operator
sorted_remaining_feat =   sorted(remaining_feat_and_rank, key=operator.itemgetter(1))        
NumberOfBatchesToConsider = 2
bacthOfTogether = 100
counterToList = 0
for i in range(0,NumberOfBatchesToConsider):
    dataset_train_sel_fea["f_less_var_together_xgboost_importance_wise_"+str(i)] = 0    
    for j in range(0,bacthOfTogether):
        #print "=>" + f_importance_wise[counterToList]
        dataset_train_sel_fea["f_less_var_together_xgboost_importance_wise_"+str(i)] = dataset_train_sel_fea["f_less_var_together_xgboost_importance_wise_"+str(i)] + dataset_train_sel_fea[sorted_remaining_feat[counterToList][0]]
        counterToList = counterToList + 1
    #print "==========="
#============less varienced over =======================

NumberOfBatchesToConsider = 20
bacthOfTogether = 5
counterToList = 0
for i in range(0,NumberOfBatchesToConsider):
    dataset_train_sel_fea["loc_together_xgboost_importance_wise_"+str(i)] = 0   
    dataset_test_sel_fea["loc_together_xgboost_importance_wise_"+str(i)] = 0   
    for j in range(0,bacthOfTogether):
        #print "=>" + f_importance_wise[counterToList]
        loc_name = loc_impotance_wise[counterToList]
        dataset_train_sel_fea["loc_together_xgboost_importance_wise_"+str(i)][dataset_train_sel_fea[loc_name]==1] = 1
        dataset_test_sel_fea["loc_together_xgboost_importance_wise_"+str(i)][dataset_test_sel_fea[loc_name]==1] = 1
        counterToList = counterToList + 1
    #print "==========="
