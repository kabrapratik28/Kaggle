...
def get_data():
    train = dataset_train_sel_fea

    features = list(dataset_train_sel_fea.columns.values)

    y_train = dataset_train_ans
...
#===================== sort df frame
frameContainsMostImportantFirst = df.sort_index(by=['fscore',], ascending=[False,])
frameContainsMostImportantFirst_to_list = frameContainsMostImportantFirst.values.tolist()

#dump via pickle
import pickle
with open("feat_xgboost_importance_", 'wb') as f:
    pickle.dump(frameContainsMostImportantFirst_to_list, f)
